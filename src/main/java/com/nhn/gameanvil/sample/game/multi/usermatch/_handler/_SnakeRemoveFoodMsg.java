 ...
 */
public class _SnakeRemoveFoodMsg implements RoomMessageHandler<SnakeRoom, GameUser, GameMulti.SnakeFoodMsg> {

    private static final Logger logger = getLogger(_SnakeRemoveFoodMsg.class);

    @Override
    public void execute(SnakeRoom room, GameUser user, GameMulti.SnakeFoodMsg snakeRemoveFoodMsg) throws SuspendExecution {
        if (snakeRemoveFoodMsg != null) {
            ...
            if (snakeRemoveFoodMsg.getIsDelete()) { // 삭제
                ...
                for (GameUser gameUser : room.getGameUserMap().values()) {
                    ...
                    gameUser.send(snakeRemoveFoodMsg);
                }
            }
        } else {
